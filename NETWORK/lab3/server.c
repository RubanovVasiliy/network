...
int main() {
...

  {
    ...
    if (getsockname(socket_d, (struct sockaddr *)&server_address, &address_length) < 0) {
  ...
  while (1) {
    if ((socket_client = accept(socket_d, (struct sockaddr *)&client_address, &address_length)) < 0) {
...
void *childWork(void *args) {
  ...
  while (1) {
    if (recv(socket_client, &n, sizeof(int), 0) <= 0) {
      break;
    }
